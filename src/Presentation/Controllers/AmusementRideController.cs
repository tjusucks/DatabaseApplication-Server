...
public class AmusementRidesController : ControllerBase
{
    ...
    /// </summary>    
    /// <param name="searchTerm">Search term for ride name or description.</param>    
        ...
        [FromQuery] int pageSize = 10)
    {
        var result = await _mediator.Send(new SearchAmusementRidesQuery(
            searchTerm, status, location, managerId, minCapacity, maxCapacity, page, pageSize));
        return Ok(result);
    }
